...
def _check_config():
    ...
    if not NOTION_KEY or not DATABASE_ID:
        return {"success": False, "error": "Notion not configured — NOTION_API_KEY or NOTION_DATABASE_ID missing from .env"}
    return None
...
async def gtd_complete_task(task_id: str) -> Dict[str, Any]:
    ...
    Sets Status=Done. GTD Tag is left unchanged (DONE is not a valid select option).
    """
    return await update_notion_task(task_id=task_id, status="Done")
